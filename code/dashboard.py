...
def layout_prec_rec(test_pred,model):
    ...
    title_string = f'PR Curve - {model_name} -- PR AUC: {round(pr_auc_test,3)}<br>'     # Title of the plot


    # Layout of the plot

    layout = go.Layout(
                    title=dict(text=title_string, font=dict(size=30,color='white')),
                    xaxis={'title':'Recall',
                            'titlefont' : dict(
    ...
    return dcc.Graph(figure = go.Figure(data = [lineData,dotted_line],layout = layout),style={'height':'100%'})        
...
def layout_roc(test_pred,model):
    ...
    title_string = f'ROC Curve - {model_name} -- ROC AUC: {round(roc_auc_test,3)}<br>'  # Title of plot
...
def layout_feature_imp(model,model_name):
    ...
    # Layout for graph
    layout = go.Layout(
                    title=dict(text=f'Feature Importance - {model_name}', font=dict(size=30,color='white')),
    ...
    return dcc.Graph(figure = go.Figure(data = barData,layout = layout),style={'height':'100%'})
...
def generate_output(features,labels,model_1,model_2):
    ...
    output_data = html.Div(style={'backgroundColor': '#111111'}, children=[
        ...
        html.Div(style={'backgroundColor': '#111111','horizontalAlign':'center','textAlign':'center','display':'flex','height':'40vh'},children=[
        ...
        html.Div(style={'backgroundColor': '#111111','horizontalAlign':'center','textAlign':'center','display':'flex','height':'50vh'},children=[
